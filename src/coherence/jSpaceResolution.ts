import { GeometryEvalGrad, GeometryState, JSpaceResolution, Vector3 } from "./types";
...
const evaluateFromState = (state: GeometryState, s: Vector3): number => {
  ...
  return c; // If no Q and b, return constant term
};
...
const gradientFromState = (state: GeometryState, s: Vector3): Vector3 => {
  ...
  if (state.T) {
    ...
    return cubicGrad as Vector3;
  }
  if (Q && b) {
    // Gradient of quadratic form: grad J(s) = Q s + b
    const Qs = Q.map(row => row.reduce((sum, Qij, j) => sum + Qij * s[j], 0));
    return Qs.map((Qsi, i) => Qsi + b[i]) as Vector3;
  }
  return [0, 0, 0]; // If no Q and b, gradient is zero
};
