...
export interface PeerRegistryInterface {
  ...
  save(): Promise<void>;
  registerPeer(peer: Peer): Promise<void>;
  getPeer(origin: string): Peer | undefined;
  getAllPeers(): Peer[];
  getTrustedPeers(minTrustLevel?: number): Peer[];
  updateLastSeen(origin: string): Promise<void>;
  updateTrustLevel(origin: string, trustLevel: number): Promise<void>; 
  removePeer(origin: string): Promise<boolean>;
  ...
  cleanupStalePeers(staleAfterMs?: number): Promise<number>;
