...
if ((result.status ?? 1) === 0) {
  const rawJsonl = path.join(__dirname, "..", `data/compare_diagnostics${laneSuffix}.jsonl`);
  ...
  const deltaOut = path.join(
    __dirname,
    "..",
    `data/compare_diagnostics${laneSuffix}.delta.md`,
  );
  spawnSync(
    process.execPath,
    [
      ...
      deltaOut,
      "--title",
      "QWormhole Compare Bench Delta Report",
    ],
    { stdio: "inherit", env },
  );
}
